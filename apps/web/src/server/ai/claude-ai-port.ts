...
const SUPPORTED = new Set<AiTaskKey>([
  "daily-brief.compose",
  "health.explain",
  "stakeholder.update",
  "email.digest",
...
/**
 * Live Claude adapter for the generative tail. Prompts Claude for contract-shaped
 * JSON, validates it against the @pma/contracts schema, and enforces grounding
 * (every cited id must appear in the input). On any failure — no output, malformed
 * JSON, contract violation, or ungrounded citations — it delegates to a deterministic
 * fallback AIPort so the app stays correct and always grounded.
 */
export class ClaudeAIPort implements AIPort {
  constructor(
    private readonly client: Anthropic,
    private readonly fallback: AIPort,
  ...
  async run(task: string, input: unknown): Promise<AIResult> {
    ...
    try {
      return await this.compose(task as AiTaskKey, input);
    } catch {
      // Deterministic, always-grounded fallback keeps output correct on any Claude/validation failure.
      return this.fallback.run(task, input);
    }
  }

  private async compose(task: AiTaskKey, input: unknown): Promise<AIResult> {
    ...
    if (!parsed.success) throw new Error(`Claude output for '${task}' failed contract validation`);
    ...
    if (groundedOn.length === 0) {
      throw new Error(`Claude output for '${task}' cited no ids present in the input`);
    }
